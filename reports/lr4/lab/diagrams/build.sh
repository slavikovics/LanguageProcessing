...
#
#  database-erd в этой лабораторной шире, чем стандартный лимит PlantUML
#  в 4096px (после чего рендер молча обрезает картинку) -- поэтому лимит
#  явно поднят через PLANTUML_LIMIT_SIZE.
# =====================================================================
set -euo pipefail
...
if [ -n "${PLANTUML_CMD:-}" ]; then
  ...
  docker run --rm -e PLANTUML_LIMIT_SIZE="$PLANTUML_LIMIT_SIZE" -v "$SCRIPT_DIR:/diagrams" plantuml/plantuml \

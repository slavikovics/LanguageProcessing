...
class TranslationTestRunService:
    ...
    async def start_run(
        ...
        *,
        ...
        run = await self._test_runs.create(
            collection_id=collection_id, source_lang=source_lang, target_lang=target_lang
        )
    ...
    async def list_runs_by_collection(self, collection_id: int) -> list[TranslationTestRun]:
        return await self._test_runs.list_by_collection(collection_id)
    ...
    async def run_job(self, run_id: int) -> None:
        ...
        try:
            ...
            if final is not None and final.status == TranslationTestRunStatus.CANCELLED.value:
                return
            await self._test_runs.mark_completed(run_id)
    ...
    async def get_run_summary(self, run_id: int) -> TranslationRunSummary:
        ...
        return summarize_translation_run(
            run_id=run_id, source_lang=run.source_lang, target_lang=run.target_lang, runs=runs
        )

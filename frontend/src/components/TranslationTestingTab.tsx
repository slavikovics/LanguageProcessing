...
import { DEFAULT_SOURCE_LANGUAGE, DEFAULT_TARGET_LANGUAGE, TERMINAL_TRANSLATION_TEST_RUN_STATUSES } from "@/api/types";
...
export function TranslationTestingTab({ collectionId }: { collectionId: number | null }) {
  ...
  useEffect(() => {
    ...
    if (TERMINAL_TRANSLATION_TEST_RUN_STATUSES.includes(run.status)) finishTesting();
        ...
        <CardContent className="flex flex-col gap-4 py-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="max-w-prose text-sm text-muted-foreground">
              Пословный перевод запускается на документах текущей коллекции, размеченных как
              англоязычные при кроулинге (документы на других языках пропускаются). По завершении
              среднее число слов до и после перевода, покрытие словаря и время перевода можно

...
export function SummarizationSummaryBarChart({
  summaries,
  valueOf,
  formatValue,
  ariaLabel,
  ...
  valueOf: (summary: SummarizationRunSummary) => number;
          ...
          {summaries.map((summary, i) => {
            ...
            return (
              <g key={summary.method}>
                <title>{`${SUMMARIZATION_METHOD_LABELS[summary.method]}: ${formatValue(value)}`}</title>
                ...
                >
                  {SUMMARIZATION_METHOD_LABELS[summary.method]}
                </text>
              </g>
            );
          })}
